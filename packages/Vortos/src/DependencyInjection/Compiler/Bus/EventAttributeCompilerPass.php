...
namespace Vortos\DependencyInjection\Compiler\Bus;
...
class EventAttributeCompilerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        $eventIds = $container->findTaggedServiceIds('vortos.event');
...
// same transport could have muti topics in several event class, get them all
            ...
            foreach ($eventTransports as $eventTransport) {
                ...
                // $transportId = 'vortos.transport.' . $eventTransport . '.' . $topic;
                $producerTransportId = "vortos.transport." . strtolower(str_replace(['-', ' '], '_', $eventTransport)) . ".producer";
        ...
        foreach ($transports as $transportId => [$dsnEnvName, $topic]) {
            ...
            if (!$container->hasDefinition($transportId)) {
                $container->register($transportId, TransportInterface::class)
                    ->setFactory([new Reference('vortos.transport.factory'), 'createTransport'])
                    ->setArguments([
                        $dsnEnvName,
                    [
                        // --- 1. ROUTING FALLBACK ---
                        ...
                        new Reference('vortos.messenger.serializer')
                    ]);
            }
