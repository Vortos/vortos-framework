...
class QueryHandlerPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->has('vortos.bus.query.locator')) {
            return;
        }

        $locatorDefinition = $container->findDefinition('vortos.bus.query.locator');

        $handlers = $container->findTaggedServiceIds('vortos.query.handler');
